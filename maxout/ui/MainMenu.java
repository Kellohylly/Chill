...
public class MainMenu extends GuiScreen {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		for(String name : buttons) {
			...
			this.drawCenteredString(mc.fontRendererObj, name, (width/buttons.length) * count + (width/buttons.length)/2F + 8, height - 20, hovered ? 0xff0090ff : 5636095);
			count++;
		}
		...
		GlStateManager.translate(-(width/2F), -(height/2F), 0);
		this.drawCenteredString(mc.fontRendererObj, Client.name + " v" + Client.version, width/2F, height/2F - mc.fontRendererObj.FONT_HEIGHT/2F, 5636095);
		...
		GlStateManager.translate(-(width/2F), -(height/2F), 0);
		this.drawCenteredString(mc.fontRendererObj, Client.patchNotes, 125, 43, 5636095);
		GlStateManager.popMatrix();
